...
SELECT JOB, ROUND(AVG(SAL)) "평균 급여", SUM(SAL) "급여의 합"
    FROM EMP
    GROUP BY JOB
    HAVING AVG(SAL)>=3000;
...
SELECT DNAME, ROUND(AVG(SAL)) "급여평균", SUM(SAL) "급여합계", MIN(SAL) "최소급여"
    FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO
    GROUP BY DNAME;
...
--FORD		2
--JONES		4
SELECT U.ENAME, COUNT(D.ENAME)+1 "등수"
    FROM EMP U, EMP D
    WHERE U.SAL<D.SAL(+)
    GROUP BY U.ENAME 
